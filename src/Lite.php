...
class Lite
{
    ...
    public function __construct($config = array())
    {
        ...
        $class = false !== strpos($type, '\\') ? $type : 'PhalApi\\Template\\SDK\\' . ucwords($type);
     ...
     * @return void
     */
    public function extend($rule, $callback = null)
    {
     ...
     * @return void
     */
    public function display($content, $vars = array())
    {
        if ($vars) {
            $this->data = array_merge($this->data, $vars);
        }
    ...
    private function parsePhp(&$content)
    {
        ...
        if ($this->config['tpl_deny_php'] && false !== strpos($content, '<?php')) {
            throw new BadRequestException('not allow php tag', 1);
        }
    }
    ...
    {
        if (false !== strpos($tagLib, '_')) {
            // 支持指定标签库的命名空间
            $className = $tagLib;
            $tagLib    = substr($tagLib, strrpos($tagLib, '\\') + 1);
        } else {
            $className = 'PhalApi\\Template\\SDK\\' . ucwords($tagLib);
        }
    ...
    private function parseTemplateFile($template)
    {
        ...
        throw new BadRequestException('template not exists:' . $template, 1);
    }
